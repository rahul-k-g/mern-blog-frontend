...
const Dashboard = () => {
  ...
  async function fetchData() {
    try {
      const response = await axios.get(
        `http://localhost:5050/api/v1/fetchBlogs?page=${currentPage}&search=${searchQuery}`,
        {
          headers: {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5050/api/v1/blogOperations/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        withCredentials: true,
  ...
  const deleteSelected = async () => {
    ...
    try {
      const deletePromises = selectedRows.map((row) =>
        axios.delete(`http://localhost:5050/api/v1/blogOperations/${row._id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          withCredentials: true,
